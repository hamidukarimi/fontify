...
import BulletsAnimation from "../BulletsAnimation";
...
function toSmallCaps(text) {
  return Array.from(text)
    .map((c) => smallCapsMap[c.toLowerCase()] ?? c)
    .join("");
}
...
function toSuperscript(text) {
  return Array.from(text)
    .map((ch) => superMap[ch] ?? superMap[ch.toLowerCase()] ?? ch)
    .join("");
}
...
export default function Main() {
  ...
  const [favorites, setFavorites] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("fontify:favs") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const toggleMenu = (kind, idx) => {
    if (openMenu && openMenu.kind === kind && openMenu.idx === idx)
      setOpenMenu(null);
                ...
                >
                  ...
                  >
                    <div className="text-sm leading-5 break-words">
                      ...
                      {STYLES[i].id === "boxed" ? (
                        <pre className="m-0 whitespace-pre-wrap text-sm leading-5">
                          {val}
                        </pre>
                      ) : (
                            ...
                            >
                              <div className="bg-[#313131] rounded-md shadow-lg py-2 min-w-[160px]">
                                ...
                                >
                                  ...
                                  >
                                    <path
                                      d="M6 2h10a2 2 0 0 1 2 2v18l-7-4-7 4V4a2 2 0 0 1 2-2z"
                                      stroke="#E5E7EB"
                                      strokeWidth="1.2"
                                      strokeLinecap="round"
                                      strokeLinejoin="round"
          ...
          <div className="mt-6">
            ...
            {favorites.length === 0 ? (
              <div className="text-xs text-gray-500">
                No bookmarks yet — bookmark outputs from the menu.
              </div>
            ) : (
                ...
                <AnimatePresence>
                  {favorites.map((f, idx) => (
                    ...
                    >
                      ...
                      "
                      >
                        <div className="text-sm leading-5 break-words truncate max-w-[72%]">
                          {truncate(f, 120)}
                        </div>
